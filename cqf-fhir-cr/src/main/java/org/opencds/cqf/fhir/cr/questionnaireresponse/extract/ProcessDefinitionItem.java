...
public class ProcessDefinitionItem {
    ...
    protected void processChildItem(
        ...
        if (!children.isEmpty()) {
            ...
            if (element == null) {
                ...
                if (propDef == null) {
                    request.logException("Property definition not found for '" + prop + "' in path '" + path + "'");
                    return;
                }
                if (propDef instanceof BaseRuntimeChildDatatypeDefinition datatypeDef) {
                    ...
                    element = newBase(blockDef.getChildByName(blockDef.getElementName()).getImplementingClass());
    ...
    protected HashMap<String, BaseRuntimeChildDefinition> getPropertyDefinitions(
        ...
        for (int i = 0; i < identifiers.length; i++) {
            var name = identifiers[i].split(":")[0];
            var def = targetDef.getChildByName(name);

            if (def == null) {
                try {
                    var runtimeDef = request.getFhirContext()
                        .getResourceDefinition(
                            (Class<? extends IBaseResource>) targetDef.getImplementingClass())
                        .getChildByName(name);
                    if (runtimeDef != null) {
                        def = runtimeDef;
                    }
                } catch (Exception ex) {
                    request.logException("Unable to resolve property definition for " + name + ": " + ex.getMessage());
                }
            }

            if (def == null && name.endsWith("[x]")) {
                var baseName = name.replace("[x]", "");
                var alt = targetDef.getChildByName(baseName);
                if (alt != null) {
                    def = alt;
                }
            }

            if (def == null && targetDef.getImplementingClass().getSuperclass() != null) {
                try {
                    var parentDef = request.getFhirContext()
                        .getResourceDefinition(
                            (Class<? extends IBaseResource>) targetDef.getImplementingClass().getSuperclass());
                    var parentChild = parentDef.getChildByName(name);
                    if (parentChild != null) {
                        def = parentChild;
                    }
            ...
            if (def == null) {
                request.logException("Unable to resolve child property: " + name + " in " + targetDef.getName());
                continue;
            }

            propertyDefs.put(name, def);

            if (def instanceof BaseRuntimeDeclaredChildDefinition declaredChild) {
                try {
                    Class<?> fieldType = declaredChild.getField().getType();
                    if (IBaseBackboneElement.class.isAssignableFrom(fieldType)
                        || IBase.class.isAssignableFrom(fieldType)) {
                        targetDef = request.getFhirContext().getElementDefinition(
                            (Class<? extends IBase>) fieldType);
                    }
                } catch (Exception ignored) {
                }
            }

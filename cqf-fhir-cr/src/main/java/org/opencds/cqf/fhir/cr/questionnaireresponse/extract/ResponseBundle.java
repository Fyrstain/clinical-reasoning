...
public class ResponseBundle {
    ...
    public static IBaseBundle createBundleR4(String extractId, List<IBaseResource> resources) {
        var newBundle = new org.hl7.fhir.r4.model.Bundle().setType(
            org.hl7.fhir.r4.model.Bundle.BundleType.TRANSACTION);
        newBundle.setId(extractId);
        // ensure entry array
        newBundle.getEntry();
        resources.forEach(
            resource -> newBundle.addEntry(new org.hl7.fhir.r4.model.Bundle.BundleEntryComponent()
                .setFullUrl(generateFullUrl(resource))
    ...
    public static IBaseBundle createBundleR5(String extractId, List<IBaseResource> resources) {
        var newBundle = new org.hl7.fhir.r5.model.Bundle().setType(
            org.hl7.fhir.r5.model.Bundle.BundleType.TRANSACTION);
        newBundle.setId(extractId);
        // ensure entry array
        newBundle.getEntry();
        resources.forEach(
            resource -> newBundle.addEntry(new org.hl7.fhir.r5.model.Bundle.BundleEntryComponent()
                .setFullUrl(generateFullUrl(resource))
    ...
    private static String generateFullUrl(IBaseResource resource) {
        if (resource.getIdElement().getIdPart() != null) {
            return "http://example.org/fhir/" + resource.fhirType() + "/" + resource.getIdElement()
                .getIdPart();
        } else {
